...
# Base directories
SCRIPT_DIR = Path(__file__).resolve().parent          # 1D/ParSac
ROOT_DIR = SCRIPT_DIR.parent                          # 1D
    ...
    def add_do():
        experiment.add_target(
            sim.request_comparison(
                ...
                str(OBS_DIR/"DO_mgm3.obs"),         # obs:   mg O2 / m³
    ...
    def add_po4():
        experiment.add_target(
            sim.request_comparison(
                ...
                str(OBS_DIR/"PO4_mmolm3.obs"),      # obs:   mmol P / m³
                obs_file_format=TextFormat.DEPTH_EXPLICIT,
            )
        )
...
def build_experiment(
    ...
    sim = parsac.job.gotm.Simulation(str(ROOT_DIR), executable=str(Path(ROOT_DIR) / executable))
...
# --- DEBUG ---
    ...
    print("Executable path:", ROOT_DIR / args.exe)
    print("Exists?", (ROOT_DIR / args.exe).exists())
    # ---------------
