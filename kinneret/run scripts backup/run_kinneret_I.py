...
def create_domain(
    ...
    **kwargs,
    ...
    with netCDF4.Dataset(args.bathymetry_file) as nc:
        ...
        var = nc[args.bathymetry_name]
        fill_value = getattr(var, "_FillValue", 1.0e37)
        print(f"Loaded bathymetry: {args.bathymetry_file}, variable: {args.bathymetry_name}, FillValue: {fill_value}")
        domain = pygetm.domain.create_cartesian(
            ...
            lat=nc["latitude"][:],
            H=-1.0 * var[:, :],
            mask=np.where(var[...] == fill_value, 0, 1),
            z0=0.01,
        )
...
def create_simulation(
    ...
    **kwargs,
    ...
    sim.airsea.sp.set(pygetm.input.from_nc(ERA_path, "sp") + 5860)
    sim.airsea.tcc.set(pygetm.input.from_nc(ERA_path, "tcc") + 0.3)
    ERA_path = "ERA5/precip_????.nc"
    sim.airsea.tp.set(pygetm.input.from_nc(ERA_path, "tp") / 3600.0)

    for river in sim.rivers.values():
        fname = f"Rivers/inflow_q_{river.name}.nc"
        river.flow.set(pygetm.input.from_nc(fname, "Flow"))
    ...
    # Try to load Temp and Salt if they exist
        ds = xr.open_dataset(fname)
        if "Temp" in ds:
           river["temp"].set(pygetm.input.from_nc(fname, "Temp"))
        else:
           print(f"[WARNING] Temp missing in inflow '{river.name}' — defaulting to 20°C")
           river["temp"].set(20.0)

        if "Salt" in ds:
           river["salt"].set(pygetm.input.from_nc(fname, "Salt"))
        else:
           print(f"[WARNING] Salt missing in inflow '{river.name}' — defaulting to 0 PSU")
           river["salt"].set(0.0)
...
if __name__ == "__main__":
    ...
    domain = create_domain(args, args.runtype, args.rivers)
